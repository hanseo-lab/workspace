...
public class ArrayMemory {
	...
	public static void main(String[] args) {
		...
		//기본적으로 자바는 메모리으 주소를 확인할 수 없다.
		//객체 메모리 주소의 암호화된 해시값을 haschCode()로 가져올 수 있다.
		...
		System.out.println(iArr); //자료형 + @ + 주소값을 활용한 16진구 해시값
		iArr = null;
		System.out.println(iArr);
		
		//. : 접근연산자
		//iArr.hashCode();
		//NullPointerExeption - > 참조변수가 가지고 있는 주소값이 없다.
		
		int[] arr = new int[5];
		//System.out.println(arr[5]);
		//ArrayIndexOutOfBoundsExcepiton
		//배열의 크기를 벗어난 index를 제시하면 발생하는 에러
		...
		/*
		 * 연결이 끊어진 기존 배열의 메모리공간은 Heep영역에 남아있다.(어떤 변수에도 참조되지않은 상태)
		 * => 일정시간이 지나면 "GC(가비지컬렉터)"가 알아서 회수한다.
		 * => 자바에서 "자동메모리관리" 특징 => 개발자는 코드에 집중할 수 있음.
		 */
