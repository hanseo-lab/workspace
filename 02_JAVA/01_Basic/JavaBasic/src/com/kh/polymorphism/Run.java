...
public class Run {
	...
	public static void main(String[] args) {
		 ...
		 *  실제로 어디까지 접근 가능한가  =  실제로 어떤 형태의 메모리가 생성되는가
		 */
		
		/*
		Cake c1 = new CheeseCake();
		//Cake까지 접근가능하며 메모리공간은 실제로 CheeseCake의 공간
		c1.sweet();
		...
		c2.moveAvante();
		//부모참조변수로 업케스팅 가능 -> 오버라이딩된 메서드는 생성된 메모리를 기준으로 호출됨.
		((Car)c2).drive();
		//((Car)c2)..moveAvante(); //업케스팅시 자식요소의 메모리 접근이 불가.
		
		//3. 부모타입 래퍼런스로 자식객체를 다루는 경우(업캐스팅)
		Car c3 = new Sonata("검정", "디젤", 2023);
		//c3.moveSonata(); 부모타입의 참조변수이기 때문에 자식요소의 매서드 접근 불가.
