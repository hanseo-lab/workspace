...
public class TCPServer {
	/*
	 * TCP - 서버, 클라이언트와 1:1 소켓통신 - 데이터를 교환하기에 앞서, 서버, 클라이언트 연결이 되어있어야한다. (서버가 먼저
	 * 실행되어있어야 클라이언트의 연결요청을 받을 수 있음)
	 * 
	 * Socket - 프로세스간에 통신을 담당 - input/outputStream을 가지고 있음(해당 스트림을 통해 데이터를 입출력 가능)
	 * 
	 * ServerSocket - 포트와 연결되어 외부의 연결요청을 기다림 -> 요청이 들어오면 수락 수락 -> 통신할 수 있는 socket생성
	 */
