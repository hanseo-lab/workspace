...
public class Variable {
	...
	public static void main(String[] args) {
		...
		//--------------------------------------------------------
		// 원시타입 : 가장 기본적인 데이터타입, 값 자체를 저장하여 추가적인 속서잉 없다.
		 ...
		 * 문자열
		 * String 
		 * 문자열 같은 경우 할당할 메모리크기가 가변적으로 원시타입이 아닌 객체타입으로 변수를 만들어 사용함.
		 * 
		 * 원래 객체를 만드는 방식
		 ...
		 * ex) UserName, TotalPrice
		 * 
		 * 스네이크케이스 : 문자와 문자사이를 언더스코어(_)를 통해서 이어주는 것(파이선에서 주로 사용)
		 * ex) user_name, total_price
		 * 
		 * 대문자스네이크케이스 : 스네이크케이스 + 모두 대문자(대부분 언어에서 상수명)
		 * ex) USER_NAME
		 * 
		 * 케밥케이스 : 단어를 소문자로 작성하되 문자와 문자사이를 하이픈(-)으로 이어주는 것(html속성, nul, 설정파일등)
		 * ex)user-name
		 * */
	}
