...
--4. EMPLOYEE 테이블의 직원명, 이메일, 전화번호, 고용일 
...
-- 코드 실행시 현재 날짜를 표시하는 상수: SYSDATE[년/월/일/시/분/처]
...
/*
<DISTINCT>
중복 제거 - 컬럼에 표시된 값들을 한번씩만 조회하고자 할 때 사용
*/
...
--FROM EMPLOYEE;
-- 위처럼 사용시 에러가 발생한다. DISTINCT한 명령에서 한번만 사용가능

-- DISTINCT는 항상 ROW데이터 전체에 대해서 중복 제거한다.
...
조건A OR 조건B -> 조건A와 조건B 중에 하나만 만족해도 참으로 한다.
...
-- NOT : 논리부정 연산자
-- 칼럼명앞에 또는 BETWEEN앞에 선언 가능

SELECT EMP_NAME, EMP_ID, SALARY
FROM EMPLOYEE
WHERE NOT SALARY BETWEEN 3500000 AND 6000000;

--입사일이 '90/01/01'이상 '01/01/01이하인 사원들을 전체 조회
SELECT *
FROM EMPLOYEE
--WHERE HIRE_DATE >= '90/01/01' AND HIRE_DATE <= '01/01/01';
WHERE HIRE_DATE BETWEEN '90/01/01' AND '01/01/01';

--NULL을 비교연산할 때 =을 사용할 수없다
--NULL을 비교연산할 때는 IS NULL, IS NOT NULL을 사용한다.
SELECT * 
FROM EMPLOYEE
-- WHERE BONUS IS NULL;
...
/*
 ...
 EX) 비교할 대상 컬럼 LIKE '_문자': 비교할 대상컬럼 값 문자앞에 아무글자나 딱 한 글자가 있는 값을 조회
     ...
     비교할 대상 컬럼 LIKE '_문자___': 내가 원하는 형태로 _를 통해서 문자수를 조절할 수 있다.
 */
...
--사원들 중에서 전화번호가 3번째자리가 1인 사원들 사번, 사원명, 전화번호 조회
...
--와일드카드로 직접 문자로 사용할 때는 일반문자로 구분을 해줘야한다.
...
--5. 사수(MENAGER)가 없고 부서배치도 받지 않은 사원들의 사원명, 사번, 부서코드 조회
...
--정렬기준에 컬럼값이 동일한 경우 그 다음차순을 위해서 여러개의 기준 컬럼을 제시할 수 있다.
