/*
    ...
    오라클에서 객체(구조) : 테이블, 뷰, 시퀀스, 인덱스, 패캐지, 트리거, 프로시저, 함수, 사용자 등...
...
/*
    ...
    *자료형
    ...
    - 문자(CHAR(바이트크기) | VARCHAR2(바이트크기)) -> 반드시 크기를 지정해야 함
        ...
        - VARCHAR2 : 최대 4000바이트가지 지정가능 / 가변길이(몇글자의 데이터가 들어올지 확신할 수 없는 경우)
        - 날짜(DATE)
*/

--회원에 대한 데이터를 담기위한 테이블 MUMBER 생성
CREATE TABLE MEMBER (
    MEM_NO NUMBER,
...
COMMENT ON COLUMN MEMBER. MEN_NO IS '회원번호';
COMMENT ON COLUMN MEMBER. MEN_ID IS '회원아이디';
COMMENT ON COLUMN MEMBER. MEN_PWD IS '회원비밀번호';
COMMENT ON COLUMN MEMBER. MEN_NAME IS '회원이름';
...
/*
    *NOT NULL
    ...
    1. 컬럼레벨 제약 조건부여
    CREATE TABLE MEMBER(
        NEME VARCHAR2(30) PRIMARY KEY
    )
    
    2. 테이블레벨 제약조건 부여
    CREATE TABLE MEMBER(
        NEME VARCHAR2(30),
        CONSTARINT MEM_PK PRIMARY KEY(NAME)
    )
*/
...
/*
    *UNIQUE 제약조건
    ...
    컬럼값이 중복값을 제한하는 제약조건.
    삽입/수정시 기존에 있는 데이터 중 중복값이 있을 경우 오류를 발생시킴.
...
INSERT INTO MEMBER
VALUES(1,'USER02', 'PASS02', '홍길순', '왜', NULL, NULL, NULL );
...
CREATE TABLE MEMBER(
    ...
    MEM_ID VARCHAR2(30) CONSTRAINT MEMID_NT NOT NULL CONSTRAINT MEMID_UQ UNIQUE, --user01, test1234
    MEM_PWD VARCHAR2(30) CONSTRAINT MEMPWD_NT NOT NULL,
    MEM_NAME VARCHAR2(21) CONSTRAINT MEMANE_NT NOT NULL,
...
INSERT INTO MEMBER
VALUES(1,'USER02', 'PASS02', '홍길순', '여', '010-1111-2222', 'bbb@naver.com', '25/07/30' );
...
/*
    ...
    EX) 회원번호, 학버느 부서코드, 주민등록번호, 택배운송장번호...
    기본키 -> 중복금지(UNQIUE) + NULL금지(NOT NULL)
    식별, 무결, 연결
...
CREATE TABLE MEMBER (
    MEM_NO NUMBER CONSTRAINT MEMNO_NT PRIMARY KEY,
    MEM_ID VARCHAR2(30) CONSTRAINT MEMID_NT NOT NULL CONSTRAINT MEMID_UQ UNIQUE, --user01, test1234
    MEM_PWD VARCHAR2(30) CONSTRAINT MEMPWD_NT NOT NULL,
    MEM_NAME VARCHAR2(21) CONSTRAINT MEMANE_NT NOT NULL,
    GENDER CHAR(3)CHECK(GENDER IN ('남', '여')),
    ...
    CREATE_AT DATE,
    CONSTRAINT EMAIL_UQ UNIQUE(EMAIL) --테이블레벨 부여방식
    --CONSTRAINT MEMNO_NT PRIMARY KEY(MEM_NO) 테이블레벨 부여방식
);
...
INSERT INTO MEMBER
VALUES(NULL,'USER03', 'PASS03', '홍길삼', NULL, '010-1111-3333', 'aaa3@naver.com', '25/07/30' );
--기본키에 중복된 값을 담으려고할 때 (NOT NULL제약조건 위반)
...
--복함키: 두개의 컬럼을 동시에 하나의 PRIMARY KEY로 지정하는 것;
...
/*
    ...
    다른 테이블의 기본기(PK) 또는 고유키(UNIQUE)를 참조하도록 설정하는 제약조건
    -> 다른 테이블을 참조한다고 표현
...
--유효하지않은 회원등급번호는 정상적을 INSERT할 수 없음.
INSERT INTO MEM
VALUES(5, 'USER5', 'PASS5', '홍길오', NULL, NULL, NULL, '25/01/01', 50);
...
WHERE GRADE_CODE = 10;
--MEM테이블에서 10이라는 GRADE_ID가 있기때문에 부모테이블의 GRADE_CODE 10을 가진 데이터가 삭제가 안됨.

DELETE FROM MEM_GRADE
WHERE GRADE_CODE = 30;
--MEM테이블에서 참고하지있지 않은 30의 GRADE_CODE를 가진 데이터는 삭제가 가능함.

--자식테이블에서 이미 사용하고 있는 값이 있을 경우
--부모테이블로부터 무조건 삭제가 안되는 "삭제제한"옵션이 기본값을 설정되어있음.
...
/*
    ...
    *삭제옵션 : 부모테이블의 데이터 삭제시 그 데이터를 참조하고있는 자식테이블의 데이터를 어떻게 처리할 것인가?
    ...
    - ON DELETE RESTRLCTED(기본값) : 삭제제한옵션, 자식데이터로부터 쓰이는 부모데이터는 삭제가 안됨.
...
DELETE FROM MEM_GRADE
WHERE GRADE_CODE = 40;
-- MEMM_GRADE의 PK 40데이터가 삭제되면 SET NULL옵션에 따라
-- GRADE_CODE 40인 데이터를 참조하던 자식데이터들은 전부 NULL을 가짐
...
DELETE FROM MEM_GRADE
WHERE GRADE_CODE = 10;
-- MEMM_GRADE의 PK 10데이터가 삭제되면 CASCADE옵션에 따라
-- 자식데이터 중 GRADE_CODE 10을 참조하던 데이터는 모두 함께 삭제된다.

-----------------------------------------------------------------------------
/*
    <DEFAULT 기본값> *제약조건이 아니다.
    컬럼을 선정하지않고 INSERT시에 NULL이 아닌 기본값을 INSERT하고자 한다.
    미리 값을 세팅해 둘 수 있음.
...
VALUES(4, 'USER04',27);
--> 선택되지 않은 컬럼에서 기본적으로 NULL이 적요됨
--> 단, 해당 컬럼에 DEFULT값이 부여되어있다면 NULL이아닌 DEFAULT값이 적용됨
------------------------------------------------------------
--관리자계정으로 계정생성
...
--KH계정을 전환
--------------------------------------------------------------------------------
--테이블을 복제할 수 있다. 여기서부터 KH계정을 진행함,
CREATE TABLE EMPLOYEE_COPY
AS (SELECT * FROM EMPLOYEE);

--CREATE TABLE 테이블명 AS SELECT문
-- AS 뒤에오는 SELECT문을 결과를 기반으로 테이블을 생성하겠다.
...
--EMPLOYEE_COPY 테이블에 DEPT_CODE에 외래키제약조건을 추가(DEPT_ID를 참조)
