...
/*
    *서브쿼리 구문
    ...
    >> 서브쿼리의 결과값에 따라서 서브쿼리 앞족에 연산자가 달라진다.
*/
...
--1) 유재식 또는 윤은해 사원과 같은 지급인 사원들의 사번, 사원명, 직급코드, 급여 조회
SELECT EMP_ID, EMP_NAME, JOB_CODE, SALARY
FROM EMPLOYEE
...
--2) 대리직급에도 과장직급의 최소급여보다 더 많이 받는 사원들의 사번, 이름, 직급, 급여 조회
...
--ROWNUM : 오라클에서 기본적을 제공해주는 컬럼, 조회된 순서대로 1부터 순번을 부여해줍니다.
...
--부서코드, 부서별 총 급여합, 부서별 평균 급여, 부서별 사원수 조회
