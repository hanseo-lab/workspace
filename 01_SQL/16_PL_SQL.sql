/*
    ...
    일반 SQL의 단점을 보완해서 변수의 정의, 조건(IF), 반복(FOR, WHILE)등을 지원하여 다수의 SQL문을 한번에 실행할 수 있음.
    PL/SQL은 블록구조를 사용함.
    
    PL/SQL 구조
    DECLARE   -- (선택)선언부 : 변수, 상수등을 선언
    BEGIN     -- 실행부 : SQL문 또는 제어문드으이 로직을 기술
    EXCEPTON  -- (선택)예외처리부 : 오류 발생시 처리
    END;      -- PL/SQL의 마지막에 기술
*/
...
----------------------------------------------------------------
/*
    1_2) 래퍼런스 타입 변수 : 이떤 테이블의 어떤 데이터타입을 참조해서 사용할지를 정하는 방식    
*/
DECLARE
...
--래퍼런스 변수 (EID, ENAME, DTITLE, NOCDE)를 생성하고
--각 테이블에서 EMP_ID, EMP_NAME, DEPT_TITLE, NATIONAL_CODE를 참조해라
--일반타입변수 TEAM을 문자열타입을 생성하고 사용자가 입력한 사원정보를 가져와서
--사번, 이름, 부서명, 근무국가코드 조회 후에 각 변수에 대입.
--NCODE값이 KO일 경우 ->  TEAM = 국내팀
--NOCDE값이 KO가 아닐경우 -> TEAM = 해외팀 대입
...
-- I에 10부터 1인 역순으로 부여해서 각각 반복문내의 코드를 실행함
BEGIN
...
/*
    ...
    *시스템 예외(오라클 미리 설정해둔 예외)
    -NO_DATA_FOUND : SELET한 결과가 한행도 없을 때
    -TOO_MANY_ROWS : SELET한 결과가 여러행일 경우
    -ZERO_DIVIDE : 0으로 나눌경우
    -DUP_VAL_ON_INDEX : UNIQUE제약조건 위배
    ...
*/

--사용자가 입력한 수를 나눗셈한 결과를 출력
DECLARE
    RESULT NUMBER;
BEGIN
    RESULT := 10/&숫자;
...
--    WHEN ZERO_DIVIDE THEN DBMS_OUTPUT.PUT_LINE('나누기 연산자는 0으로 나눌 수 없습니다');
    WHEN OTHERS THEN DBMS_OUTPUT.PUT_LINE('에러가 발생하였습니다.');
END;
/
