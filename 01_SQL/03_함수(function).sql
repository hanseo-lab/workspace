/*
   ...
   - 그룹함수 : N개의 값을 읽어들여서 1개의 결과값을 리턴(그룹지어 그룹별로 함수 실행 결과를 반환)
 ...
 *INSTR
 ...
 INSTR(컬럼 | '문자열', '찾고자하는 문자, ['찾을 위치의 시작값', 순번]) -> 결과는 NUBER
 
*/
SELECT INSTR ('AAABAACAABBAA', 'B') FROM DUAL;
--찾을 위치의 시작 값: 1, 순번: 1 -> 기본값
SELECT INSTR ('AAABAACAABBAA', 'B', 1) FROM DUAL;
SELECT INSTR ('AAABAACAABBAA', 'B', -1) FROM DUAL;
SELECT INSTR ('AAABAACAABBAA', 'B', 1, 2) FROM DUAL;
SELECT INSTR ('AAABAACAABBAA', 'B', 1, 3) FROM DUAL;
...
--30만큼의 길이 중 EMAIL컬럼값은 오른쪽으로 정령하고 나머지 부분은 공백으로 채운다.
...
SELECT LTRIM(' K H ') FROM DUAL; --앞에서부터 다른 문자가 나올 때까지 공백을 제거
SELECT RTRIM('123123KH123','123') FROM DUAL;
SELECT LTRIM('ACABACCAKHBA ','ABC') FROM DUAL; --제거하고자 하는 문자는 문자열이 아닌 문자들
...
SELECT TRIM(BOTH 'Z' FROM 'ZZZZZK HZZZ') FROM DUAL;
SELECT TRIM(LEADING 'Z' FROM 'ZZZZZK HZZ') FROM DUAL;
SELECT TRIM(TRAILING 'Z' FROM 'ZZZZZK HZZZ') FROM DUAL;
...
SELECT ROUND (123.456, 1) FROM DUAL; --위치값이 양수로 증가할수록 소수점 뒤로 한칸식 이동한다.
SELECT ROUND (123.456, -1) FROM DUAL; --위치값이 음수로 감소할수록 소수점 뒤로 한칸식 이동한다.
...
SELECT TO_CHAR(2000000, 'L9999999') FROM DUAL; --현재 설정된 나라의 로컬 화쳬단위를 나타냄
...
------------------------------------------------------------------
/*
    TO_DATE : 숫자티입 또는 문자타입을 날짜티입으로 변경하는 함수
...
--------------------------------------------------------------------------------
/*
    TO_NUMBER: 문자타입의 데이터를 숫자타입으로 변환해서 변환.
    
    [표현법]
    TO_NUMBER(문자, [포맷])
*/

SELECT TO_NUMBER('06543196584156') FROM DUAL;
...
-- 직급이 J7은 급여를 15%인상(SALARY *1.15)
-- 직급이 J7은 급여를 20%인상(SALARY *1.2)
...
/*
    CASE문
    SQL에서 조건에 띠리 값을 분기하고 싶을 때 사용하는 구문
...
SELECT EMP_NAME, SALARY,
        CASE
            WHEN SALARY >= 50000000 THEN '시니어'
            WHEN SALARY >= 35000000 THEN '미들'
            ELSE '주니어'
        END
FROM EMPLOYEE;
...
--COUNT(컬럼) : 제시한 컬럼값이 NULL이 아닌 것만 행의 수를 세서 반환
...
--보너스를 받는 사원의 수
SELECT COUNT(BONUS) --BONUS 컬럼의 값이 NULL이 아닌 데이터만 카운트를 함.
FROM EMPLOYEE;
