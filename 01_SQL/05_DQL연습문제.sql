...
SELECT EMP_NAME, SALARY*12 AS "연봉", (SALARY + (SALARY * NVL(BONUS, 0)))*12 AS "총수령액(보너스포함)", 
...
SELECT EMP_NAME, SALARY, 
    ((SALARY+(SALARY * NVL(BONUS, 0)))*12) - (SALARY*12*0.03) "실수령액", HIRE_DATE
FROM EMPLOYEE
...
--10. EMPLOYEE 테이블에 DEPT_CODE가 D9이거나 D5인 사원 중 고용일이 02년 1월 1일보다 빠른 
--    사원의 이름, 부서코드, 고용일 조회
...
SELECT SUM(SALARY + (SALARY * NVL(BONUS, 0))*12) AS "보너스 포함 연봉 합"
FROM EMPLOYEE
WHERE DEPT_CODE = 'D5';
...
SELECT COUNT(*) AS "전체 직원 수",
       ...
       COUNT(DECODE(EXTRACT(YEAR FROM HIRE_DATE), 2004, 1,NULL)) AS "20014년"
FROM EMPLOYEE;