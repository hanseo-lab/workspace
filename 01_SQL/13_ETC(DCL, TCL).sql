/*
    ...
    ORACLE은 기본적을 사용자 생성 후 권한을 부여해야 로그인및 객체사용 가능
*/
...
/*
    <TCL : 트랜잭션 제어문>
    트랜잭션은 하나이상의 DML(INSERT, UPDATE, DELETE)문을 묶어서 하나의 논리적 직업 단위로 처리하는 개념.
...
/*
    ...
    I(Isolation) 독립성 : 여러 트랜잭션이 동시에 실행되도 서로 간섭없이 독립적으로 실행되어여한다.
    D(Durability) 지속성 : 트랜잭션이 commit되면, 그 결과는 시스템이 종료되고 영구적을 보존되어야한다.
*/
